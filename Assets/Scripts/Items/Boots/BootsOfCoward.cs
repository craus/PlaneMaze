...
public class BootsOfCoward : MonoBehaviour
{
    public void Awake() {
    ...
    private async Task AfterOwnerTakeAction(MoveAction action) {
        if (action is Walk walk) {
            ...
            var target = walk.from.Shift(-moveDirection).GetFigure<Monster>(m => m.Threatening);
            if (target != null) {
                if (!walk.to.Neighbours().Any(n => n.GetFigure<Monster>(m => m.Threatening))) {
